...
>- print, len, abs 등, built-in 이라고도 한다.

### 2. 외장함수

>- random, pprint 등
>  - random.choic : random 중에서 choice를 가져다가 쓴다. 
>  - choice는 임의로 하나의 요소를 선택 해준다.
>- import를 통해 모듈을 불러온다.
...
>
> - 위와 같이 프로그램끼리 소통할 때에도 규칙이 있는데, 이것이 API 문서에 정리되어 있다.
>
> - 보통 정보를 주고 받는다고 표현한다.
...
>
> - 데이터 타입은 사용자를 위한 형식과 프로그램을 위한 형식이 있는데 원하는 형식을 가져오는 것이 편리하다.
...
>   - 위의 페이지를 각각 만들지 않고 variable routing(동적 라우팅)을 이용한다.
...
> : 글자의 폰트(구별하기 쉽게)
>
> - 개발할 때 폰트가 중요하다. I, l o, 0, O 등 대문자 소문자 구별이 필요하다.
>
...
> > - id() : 값의 메모리 주소를 확인한다.
> >
> > - 같은 값, 다른 값을 동시 할당 가능하다.
> >
> > - 변수와 값의 개수는 같아야 한다.
> >
> > - 변수 값을 바꾸는 경우 : 임의의 변경한다.
> >
> >   ```python
> >   x = 1
> >   y = 2
...
> >   - 허수부는 j로 표현
> >   - 문자열을 변환
> >   - 할때, 문자열의 중앙의 연산자 주위에 공백을 포함해서는 안된다.
> >
> > ##### 3.2 문자(string) 타입
> >
> > - 기본 활용법
...
> - if
>
>   : 참 / 거짓을 판단할 수 있는 조건과 같이 사용되어야 한다.
>
>   ```python
>   # 내 생일 출력
...
:heavy_check_mark: 구글링을 통해 문서를 찾을 때는 docs.python.org 문서 추천(파이썬 공식 문서)
...
> - 값을 대입하는 경우 동시에 일어나야 할 때는 같은 줄에 써야함!  그렇지 않으면 순서가 생겨서 값이 달라진다.
> - for 문을 2개이상 써서 비교할 때에는 상위 for 문의 인덱스 값과 비교를 하는지 생각하고 비교 안할시 if 문을 통해 continue를 시켜준다.
> - for i in len() : type 오류가 뜸, 값을 집어넣어주어야 하기 때문에 range를 써야한다.
